...
public abstract class ConversationMulticastFSM extends ConversationMulticast {

//	protected int priority;
	protected FSM fsm;

	public ConversationMulticastFSM(APlayerDataModel dm, Interpreter interpreter, String myKey, String type,
			TreeMap<String, String> to_toKey) {

		super(dm,  interpreter, myKey, type, to_toKey);
	...
	public void handle(Message msg) {
			...
			if ((msg.getToKey() == null)){
				...
				if (myKey != null){ // We are busy;				
					...
					ConversationMulticast conv = spawn(key, msg);
			...
			if (!this.to_toKey.containsKey(msg.getFrom()))
				return;
	...
	public void initiate(){
		...
		try {
			...
			System.err.println("ConversationMulticastFSM.initiate() Debug0 threw " + e );
		}
	};
	...
	public void handleFSMTransistion(Input input){
		...
		try {
			...
			System.out.println(trs.event + "/" + trs.action + "/" + trs.nextState + "/" + trs.timeout);
			...
			System.err.println("ConversationMulticastFSM.handle() Debug0 threw " + e );
		}
	}	
